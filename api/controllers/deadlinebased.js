...
class TravelRouteOptimizer {
    ...
    dijkstraminCost(origin, dest, arrTime, maxTime) {
        ...
        // Create route details JSON
        const routeDetails = {
            origin: this.cityNotoName.get(origin),
            destination: this.cityNotoName.get(dest),
            totalCost: minCost.get(dest),
            arrivalTime: this.convertMinutesToTime(minArrivalTime.get(dest)),
            route: []
        };
        ...
        for (let i = 0; i < path.length - 1; i++) {
            ...
            routeDetails.route.push({
                ...
                endTime: this.convertMinutesToTime(routeInfo.arrivalTime)
            });
        }

        return [minArrivalTime.get(dest), minCost.get(dest), routeDetails];
    }
}
