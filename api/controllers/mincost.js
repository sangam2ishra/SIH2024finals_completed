...
export const findMinCost=(origin, destination, arrivalTime)=>{
    ...
    const destinationID = router.cityNametoNo.get(destination);
    if (!originID || !destinationID) return { error: "Invalid cities" };

    const [arrivalTimeInMinutes, cost] = router.dijkstramincost(originID, destinationID, router.parseminutes(arrivalTime));
