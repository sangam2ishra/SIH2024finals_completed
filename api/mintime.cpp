...
pair<int, int> dijkstramintime(int origin, int dest, int arrtime) {
    ...
    pq.insert({arrtime, origin});

    while (!pq.empty()) {
        auto it = *pq.begin();
        int currTime = it.first, currNode = it.second;
        pq.erase(pq.find(it));
        ...
        int currCost = minArrivalTime[currNode].second;
        ...
        for (int neighbor : graph[currNode]) {
            ...
            for (const auto& edge : storage[currNode][neighbor]) {
                if (edge.arrivalTime >= currTime && edge.departureTime < minArrivalTime[neighbor].first) {
                    ...
                    pq.insert({edge.departureTime, neighbor});
                }
            }
        }
    }
    ...
    // for(auto x : outpt)cout<<x[0]<<" "<<x[1]<<" "<<x[2]<<"\n";
    for(auto x : outpt){
        ...
        cout<<"Timing : "<<convertMinutesToTime(storage2[x[0]][x[1]][x[2]].arrivalTime)<<" "<<convertMinutesToTime(storage2[x[0]][x[1]][x[2]].departureTime)<<"\n";
    }
...
int main() {
    ...
    string originstring;
    int origin = 8; // Ghaziabad (Bombay)
    int dest = 2;   // NewDelhi (Delhi)
    ...
    pair<int,int> vs=dijkstramintime(origin,dest,arrtime);
    cout<<"min time is : "<<convertMinutesToTime(vs.first)<<"\n";
    cout<<"cost for mintime is : "<<vs.second<<"\n";
