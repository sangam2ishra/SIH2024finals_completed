...
pair<int, int> dijkstramincost(int origin, int dest, int arrtime) {
    ...
    while (!pq.empty()) {
        ...
        for (int neighbor : graph[currNode]) {
            ...
            for (const auto& edge : storage[currNode][neighbor]) {
                ...
                if (edge.arrivalTime >= currTime && currCost + edge.cost < minArrivalTime[neighbor].second) {
...
int main() {
    ...
    pair<int,int> vs=dijkstramincost(origin,dest,arrtime);
